...
function validaLogin(username, userpass) {
    var bool = true;
    if (!(username)) {
        mensaje("Indique Su Usuario");
        bool = false;
        return;
    }
    if (!(userpass)) {
        mensaje("Indique Su Clave");
        bool = false;
        return;
    }
    return bool;
...
function validaNewUser(usuario, clave, confirm_clave, tipo_rif, rif, celular) {
    ...
    if (clave.length <= 5) {
        mensaje("Su Clave debe tener al menos 6 Caracteres");
        bool = false;
        return;
    }
    if (clave != confirm_clave) {
        mensaje("Error de confirmacion en su clave");
        bool = false;
        return;
    }
    if (!(tipo_rif)) {
        mensaje("Indique Tipo de RIF");
        bool = false;
        return;
    }
    if (!(rif)) {
        mensaje("Indique su RIF de Empresa");
        bool = false;
        return;
    }
    if (!(celular)) {
        mensaje("Indique su Nro. Celular");
        bool = false;
        return;
    }
    return bool;
...
function validaPago(id_banco_cuenta, id_banco, referencia, fecha_pago, monto, numero_dua) {
    var bool = true;
    if (!(id_banco)) {
        mensaje("Indique Banco");
        bool = false;
        return;
    }
    if (!(id_banco_cuenta)) {
        mensaje("Indique Nro. de Cuenta");
        bool = false;
        return;
    }
    if (!(fecha_pago)) {
        mensaje("Indique Fecha del Comprobante");
        bool = false;
        return;
    }
    if (!(referencia)) {
        mensaje("Indique Referencia del Comprobante");
        bool = false;
        return;
    }
    if (!(monto)) {
        mensaje("Indique Monto del Pago");
        bool = false;
        return;
    }
    if (!(numero_dua)) {
        mensaje("Indique Numero DUA");
        bool = false;
        return;
    }
    return bool;
...
function signup() {
    ...
    if (reg) {
        ...
        $.ajax({
            ...
            data: { username: username, userpass: userpass, cod_celular: cod_celular, celular: celular, tipo_rif: tipo_rif, rif: rif },
            success: function(data) {
                if (parserJsn == 1)
                    data = JSON.parse(data);
                if (data.estatus == 1) {
...
function formatCurrency(input, blur) {
    ...
    if (input_val.indexOf(",") >= 0) {
        ...
        // join number by .
        input_val = "Bs." + left_side + "," + right_side;
