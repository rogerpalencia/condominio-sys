...
function validaNewUser(usuario,clave,confirm_clave,tipo_rif,rif,celular){
    ...
    if (!validateEmail(usuario)) { 
        mensaje("Email No es valido");bool=false;return;}
...
function validaPago(id_banco_cuenta,id_banco,referencia,fecha_pago,monto,numero_dua){
    var bool= true;
    if(!(id_banco)){mensaje("Indique Banco");bool=false;return;}
    if(!(id_banco_cuenta)){mensaje("Indique Nro. de Cuenta");bool=false;return;}
    if(!(fecha_pago)){mensaje("Indique Fecha del Comprobante");bool=false;return;}
    if(!(referencia)){mensaje("Indique Referencia del Comprobante");bool=false;return;}
    if(!(monto)){mensaje("Indique Monto del Pago");bool=false;return;}
    if(!(numero_dua)){mensaje("Indique Numero DUA");bool=false;return;}
    return bool;
}
