...
try {
    ...
    // Compatibilidad: agrega columnas para cuenta financiera y trazabilidad de pago si no existen
    ...
    $required = ['id_notificacion_master', 'id_condominio', 'id_moneda', 'anio', 'fecha_emision', 'fecha_vencimiento', 'descripcion_cab', 'id_tipo'];
    ...
    $id_notificacion = (int)$_POST['id_notificacion_master'];
    ...
    // Verificar si la notificación existe y no está emitida
    ...
    if ($estado === 'emitida') {
        throw new Exception('No se puede editar una notificación emitida');
    }
    ...
    $sql_check = "SELECT 1 FROM notificacion_cobro_master 
                  WHERE id_condominio = :c AND anio = :a AND mes = :m AND id_tipo = :t AND estado = 'emitida' AND id_notificacion_master != :id";
    $stmt = $conn->prepare($sql_check);
    $stmt->execute([':c' => $id_condo, ':a' => $anio, ':m' => $mes, ':t' => $id_tipo, ':id' => $id_notificacion]);
    if ($stmt->fetchColumn()) {
  ...
  //      }
    ...
    // Actualizar cabecera
    $sql = "UPDATE notificacion_cobro_master SET
            id_condominio = :c, anio = :a, mes = :m, fecha_emision = :fe, 
            fecha_vencimiento = :fv, monto_total = :tot, id_moneda = :mon, 
            descripcion = :d, id_tipo = :tipo, 
    ...
    $conn->commit();
    echo json_encode(['status' => 'ok', 'id' => $id_notificacion]);
