...
WITH todas_las_fechas AS (
    SELECT to_char(to_date('$fecha_i ', 'YYYY-MM-DD') + (n || ' days')::interval, 'DD-MM-YYYY') AS fecha_pago
    FROM generate_series(0, 30) n
)

SELECT
    todas_las_fechas.fecha_pago,
    COALESCE(SUM(CASE WHEN pagos_consolidados.tipo_pago = 'CREDITO' THEN pagos_consolidados.monto_pagado ELSE 0 END), 0) AS total_pagos_credito,
    COALESCE(COUNT(CASE WHEN pagos_consolidados.tipo_pago = 'CREDITO' THEN 1 END), 0) AS cant_declaraciones_credito,
    COALESCE(SUM(CASE WHEN pagos_consolidados.tipo_pago = 'BANCO' THEN pagos_consolidados.monto_pagado ELSE 0 END), 0) AS total_pagos_bancos,
    COALESCE(COUNT(CASE WHEN pagos_consolidados.tipo_pago = 'BANCO' THEN 1 END), 0) AS cant_declaraciones_bancos
FROM
    todas_las_fechas
LEFT JOIN (
    SELECT
...
) AS pagos_consolidados ON todas_las_fechas.fecha_pago = pagos_consolidados.fecha_pago
GROUP BY
    todas_las_fechas.fecha_pago
ORDER BY
    todas_las_fechas.fecha_pago;
...
//file_put_contents('archivo_fecha', $sql);
...
while ($fila = pg_fetch_assoc($resultado)) {
    $serieA[] = $fila['cant_declaraciones_credito']??0;
    $serieB[] = $fila['cant_declaraciones_bancos']??0;

    $serieC[] = $fila['total_pagos_credito']??0;
    $serieD[] = $fila['total_pagos_bancos']??0;
            ...
            <div class="card card-h-100">
                ...
                <div class="card-body">
                    ...
                    <div class="text-nowrap">
                        ...
                        <span class="ms-1 text-muted font-size-13">Solventes en <?php echo date('Y');?> </span>
                        <span class="badge bg-soft-success text-success">
...
<div class="row">
    <div class="col-xl-12">
        <div class="card card-h-100">
            <!-- card body -->
            <div class="card-body" style="margin-top: -37px;margin-bottom: -48px;">
                <!-- card -->
    ...
    <div class="col-xl-12">
        <div class="card card-h-100">
            ...
            <div class="card-body" >
                ...
                <div class="card-body" style="margin-bottom: -43px;margin-top: -33px;">
